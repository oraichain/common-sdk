...
import { DemoStargateClient } from "@oraichain/common-contracts-sdk/src/DemoStargate.client";

const sender = "orai18hr8jggl3xnrutfujy2jwpeu0l76azprlvgrwt";
const demoContract =
  "orai19dlyf58lfzaqwkvn63xqawqq3xxyypd6l7h9rh5qjtmx8ym22nfqtups8c"; // https://github.com/oraichain/demo-stargate.git
...
async function instantiateCosmWasmClient(): Promise<SigningCosmWasmClient> {
  ...
  return SigningCosmWasmClient.connectWithSigner(
    "https://rpc.orai.io",
    wallet,
    { gasPrice: GasPrice.fromString("0.001orai"), prefix: "orai" }
  );
}

async function demoSend() {
  const cosmwasmClient = await instantiateCosmWasmClient();
  const multisigClient = new DemoStargateClient(
    cosmwasmClient,
    sender,
    demoContract
  );
